(function(root, factory) {
  ...
  function damage(g, a) { decal(g, a.x, g.groundY + 5, a.size); g.base = Math.max(0, g.base - (a.tier === 'large' ? 3 : a.tier === 'medium' ? 2 : 1)); g.shake = .7; g.flash = .55; burst(g, a.x, g.groundY, '#ff7c66', 22); burst(g, a.x, g.groundY, '#3b3433', 6); if (!g.base) { g.state = 'GAMEOVER'; g.best = Math.max(g.best, g.score); } }
  function destroy(g, index) { const a = g.enemies[index]; g.enemies.splice(index, 1); g.asteroids = g.enemies; const points = a.tier === 'large' ? 500 : a.tier === 'medium' ? 250 : 100; g.score += points * g.combo; g.combo = Math.min(4, g.combo + 1); g.comboTime = 2.5; burst(g, a.x, a.y, '#ffd078', a.tier === 'large' ? 22 : 12); burst(g, a.x, a.y, '#697277', a.tier === 'large' ? 6 : 4); g.shake = Math.max(g.shake, a.tier === 'large' ? .32 : .14); decal(g, a.x, a.y, a.size * .72);
  ...
  function update(g, keys = {}, dt = .016) { dt = Math.min(.05, Math.max(0, dt)); g.time += dt; g.ground.height = Math.round(g.height * .18); g.ground.visible = true; g.groundY = g.height - g.ground.height; g.shake = Math.max(0, g.shake - dt * 2); g.flash = Math.max(0, g.flash - dt * 2); if (g.state === 'TITLE' || g.state === 'GAMEOVER') return g;
    ...
    for (let pi = g.shots.length - 1; pi >= 0; pi--) for (let ai = g.enemies.length - 1; ai >= 0; ai--) { const p = g.shots[pi], a = g.enemies[ai]; if (Math.hypot(p.x - a.x, p.y - a.y) < a.size) { g.shots.splice(pi, 1); if (--a.hp <= 0) destroy(g, ai); else { a.hit = .14; burst(g, a.x, a.y, '#ffd078', 5); burst(g, a.x, a.y, '#79ebff', 3); } break; } }
