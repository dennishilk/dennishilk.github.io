(function(root, factory) {
  ...
  function createGame(width = 1280, height = 720, seed = 9) {
    const g = { width, height, state: 'TITLE', time: 0, countdown: 0, wave: 0, waveStarted: 0, pending: 0, spawnClock: 0, score: 0, best: 0, base: BASE_HEALTH, groundY: height - Math.round(height * .18), ground: { visible: true, height: Math.round(height * .18) }, enemies: [], asteroids: [], shots: [], particles: [], shake: 0, flash: 0, combo: 1, comboTime: 0, random: rng(seed), turret: { x: width / 2, y: height - 68, angle: -Math.PI / 2, fire: 0, recoil: 0, heat: 0 } };
    g.ship = g.turret; return g;
  }
  function reset(g) { g.state = 'COUNTDOWN'; g.countdown = 2.95; g.wave = 0; g.waveStarted = 0; g.pending = 0; g.spawnClock = 0; g.score = 0; g.base = BASE_HEALTH; g.enemies = g.asteroids = []; g.shots = []; g.particles = []; g.combo = 1; g.comboTime = 0; Object.assign(g.turret, { x: g.width / 2, y: g.height - 68, angle: -Math.PI / 2, fire: 0, recoil: 0, heat: 0 }); return g; }
  const start = reset;
  ...
  function update(g, keys = {}, dt = .016) { dt = Math.min(.05, Math.max(0, dt)); g.time += dt; g.ground.height = Math.round(g.height * .18); g.ground.visible = true; g.groundY = g.height - g.ground.height; g.turret.x = g.width / 2; g.turret.y = g.groundY + 18; g.shake = Math.max(0, g.shake - dt * 2); g.flash = Math.max(0, g.flash - dt * 2); if (g.state === 'TITLE' || g.state === 'GAMEOVER') return g;
