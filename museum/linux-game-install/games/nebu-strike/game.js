(function(root, factory) {
  ...
  // These small, deterministic descriptors move with the asteroid. They are bounded per tier
  // and deliberately replace per-frame texture generation.
  function visual(size, tier, random) { const facets = tier === 'large' ? 6 : tier === 'medium' ? 4 : 2, craters = tier === 'large' ? 4 : tier === 'medium' ? 2 : 1; return { seed: Math.floor(random() * 0x7fffffff), facets: Array.from({length: facets}, () => [random() * 6.28, .22 + random() * .52, .16 + random() * .18, random() * .28]), craters: Array.from({length: craters}, () => [random() * 6.28, .2 + random() * .42, .1 + random() * .11]), ridge: random() > .43, crackAngle: random() * 6.28 }; }
  ...
  function createGame(width = 1280, height = 720, seed = 9) {
    ...
    const g = { width, height, state: 'TITLE', time: 0, countdown: 0, wave: 0, waveStarted: 0, pending: 0, spawnClock: 0, score: 0, best: 0, base: BASE_HEALTH, groundY, ground: { visible: true, height: Math.round(height * .18) }, enemies: [], asteroids: [], shots: [], particles: [], decals: [], shake: 0, flash: 0, combo: 1, comboTime: 0, events: [], random: rng(seed), turret: { angle: -Math.PI / 2, fire: 0, recoil: 0, heat: 0 } };
    g.ship = g.turret; return g;
  }
  function reset(g) { g.state = 'COUNTDOWN'; g.countdown = 2.95; g.wave = 0; g.waveStarted = 0; g.pending = 0; g.spawnClock = 0; g.score = 0; g.base = BASE_HEALTH; g.enemies = g.asteroids = []; g.shots = []; g.particles = []; g.decals = []; g.combo = 1; g.comboTime = 0; g.events = []; Object.assign(g.turret, { angle: -Math.PI / 2, fire: 0, recoil: 0, heat: 0 }); return g; }
  const start = reset;
  function beginWave(g) { if (g.pending || g.enemies.length || g.state !== 'PLAYING') return false; g.wave++; g.waveStarted++; g.events.push({ type: 'wave' }); g.pending = Math.min(30, 5 + g.wave * 2); g.spawnClock = .12; return true; }
  ...
  function fire(g) { const t = g.turret, anchor = turretGeometry(g); if (t.fire > 0 || g.shots.length >= MAX_PROJECTILES || g.state !== 'PLAYING') return false; const x = anchor.anchorX + Math.cos(t.angle) * 34, y = anchor.anchorY + Math.sin(t.angle) * 34; g.shots.push({ x, y, px: x, py: y, vx: Math.cos(t.angle) * 760, vy: Math.sin(t.angle) * 760, life: SHOT_LIFE }); g.events.push({ type: 'fire' }); t.fire = .105; t.recoil = 1; t.heat = clamp(t.heat + .16, 0, 1); burst(g, x, y, '#fff0a1', 5); return true; }
  function decal(g, x, y, size) { if (g.decals.length >= MAX_IMPACT_DECALS) g.decals.shift(); g.decals.push({x, y, size, life: 1.35}); }
  function damage(g, a) { decal(g, a.x, g.groundY + 5, a.size); g.base = Math.max(0, g.base - (a.tier === 'large' ? 3 : a.tier === 'medium' ? 2 : 1)); g.events.push({ type: 'base' }); g.shake = .7; g.flash = .55; burst(g, a.x, g.groundY, '#ff7c66', 22); burst(g, a.x, g.groundY, '#3b3433', 6); if (!g.base) { g.state = 'GAMEOVER'; g.best = Math.max(g.best, g.score); g.events.push({ type: 'gameover' }); } }
  function destroy(g, index) { const a = g.enemies[index]; g.enemies.splice(index, 1); g.asteroids = g.enemies; const points = a.tier === 'large' ? 500 : a.tier === 'medium' ? 250 : 100; g.score += points * g.combo; g.combo = Math.min(4, g.combo + 1); g.events.push({ type: 'destroy', value: a.tier }); if (g.combo > 2) g.events.push({ type: 'combo', value: g.combo }); g.comboTime = 2.5; burst(g, a.x, a.y, '#ffd078', a.tier === 'large' ? 22 : 12); burst(g, a.x, a.y, '#697277', a.tier === 'large' ? 6 : 4); g.shake = Math.max(g.shake, a.tier === 'large' ? .32 : .14); decal(g, a.x, a.y, a.size * .72);
  ...
  function update(g, keys = {}, dt = .016) { dt = Math.min(.05, Math.max(0, dt)); g.time += dt; g.ground.height = Math.round(g.height * .18); g.ground.visible = true; g.groundY = g.height - g.ground.height; g.shake = Math.max(0, g.shake - dt * 2); g.flash = Math.max(0, g.flash - dt * 2); if (g.state === 'TITLE' || g.state === 'GAMEOVER') return g;
    ...
    for (let pi = g.shots.length - 1; pi >= 0; pi--) for (let ai = g.enemies.length - 1; ai >= 0; ai--) { const p = g.shots[pi], a = g.enemies[ai]; if (Math.hypot(p.x - a.x, p.y - a.y) < a.size) { g.shots.splice(pi, 1); if (--a.hp <= 0) destroy(g, ai); else { a.hit = .14; g.events.push({ type: 'hit' }); burst(g, a.x, a.y, '#ffd078', 5); burst(g, a.x, a.y, '#79ebff', 3); } break; } }
