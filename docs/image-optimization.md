...
python3 scripts/optimize_images.py --generate
...
`--dry-run` writes nothing. `--generate` writes candidates and a state manifest;
re-running deterministically replaces candidates from unchanged originals rather
than recompressing derivatives. `--verify` checks readability, dimensions, aspect
...
are intentionally reviewed. Use 160/320 px avatar derivatives, thumbnail widths near
