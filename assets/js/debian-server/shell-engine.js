...
export class ShellEngine {
  ...
  execute(source) {
    ...
    for(const item of commandsToRun){if(!item.tokens.length)return this.finish({stdout:[],stderr:['bash: syntax error: unexpected end of file'],exitCode:2});if(item.chain==='&&'&&last.exitCode!==0)continue;if(item.chain==='||'&&last.exitCode===0)continue;last=this.executePipeline(item.tokens);}
  ...
  executePipeline(tokens){
    ...
    let output={stdout:[],stderr:[],exitCode:0};for(let words of stages){if(!words.length)return {stdout:[],stderr:['bash: syntax error: unexpected end of file'],exitCode:2};words=words.map(x=>x.replace(/^\$\?$/,String(this.state.lastExitCode??0)).replace(/\$([A-Za-z_][A-Za-z0-9_]*)/g,(_,k)=>this.state.environment[k]??''));const alias=this.state.aliases[words[0]];if(alias){const bits=alias.split(/\s+/);words=[...bits,...words.slice(1)];}const [name,...args]=words,handler=commands[name];output=handler?handler(args,{state:this.state,fs:this.fs,stdin}):{stdout:[],stderr:[`bash: ${name}: command not found`],exitCode:127};stdin=output.stdout.join('\n')+(output.stdout.length?'\n':'');if(output.stderr.length)break;}
