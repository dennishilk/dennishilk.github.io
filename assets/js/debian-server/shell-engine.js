...
export class ShellEngine {
  constructor(state) { this.state=state; const michael=state.environment?.USER==='m.weber'&&state.environment?.HOSTNAME==='workstation';this.fs=new VirtualFilesystem(state.filesystem, { home: state.environment?.HOME, user:state.environment?.USER, clock:michael?()=>new Date(state.system.frozenEpochMs).toISOString():undefined }); }
  execute(source) {
  ...
  writeRedirect(outputFile,data,append){const home=this.state.environment?.HOME||'/home/visitor',expanded=outputFile==='~'||outputFile.startsWith('~/')?home+outputFile.slice(1):outputFile,path=(expanded.startsWith('/')?expanded:`${this.state.currentDirectory}/${expanded}`).replace(/\/+/g,'/'),existing=this.fs.get(path),owner=this.state.environment?.USER||'visitor',next=(append&&existing?.type==='file'?existing.content:'')+data,now=this.fs.clock();if(this.fs.denied(path,true))return {stdout:[],stderr:[`bash: ${outputFile}: Permission denied`],exitCode:1};if(new TextEncoder().encode(next).length>LIMITS.fileSize)return {stdout:[],stderr:[`bash: ${outputFile}: File too large`],exitCode:1};if(existing){if(existing.type!=='file')return {stdout:[],stderr:[`bash: ${outputFile}: Is a directory`],exitCode:1};if(existing.protected||existing.owner!==owner)return {stdout:[],stderr:[`bash: ${outputFile}: Permission denied`],exitCode:1};const oldBytes=new TextEncoder().encode(existing.content).length,newBytes=new TextEncoder().encode(next).length;if(this.fs.bytes()-oldBytes+newBytes>LIMITS.totalBytes)return {stdout:[],stderr:[`bash: ${outputFile}: No space left on device`],exitCode:1};existing.content=next;existing.modified=now;}else{const e=this.fs.add(path,{name:'',type:'file',owner,group:owner,mode:'-rw-r--r--',content:data,created:now,modified:now,protected:false});if(e)return {stdout:[],stderr:[`bash: ${outputFile}: ${e}`],exitCode:1};}return null;}
