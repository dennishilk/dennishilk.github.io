import { explainCommand } from './command-interpretations.js';
...
function render(data) {
  ...
  document.querySelector('#early-archive-note').hidden = data.completedSessions >= 10;
  ...
  const visibleThemes = data.themes.filter(theme => theme.visitors > 0);
  visibleThemes.forEach(theme => { const card=node('article','curiosity-card');card.append(node('h3','',theme.title),node('p','curiosity-measure',`${theme.percentage}% of visitors ${theme.description}.`));if(theme.common.length){card.append(node('h4','','Most common commands'));const list=node('ul','command-list');theme.common.forEach(item=>{const entry=node('li');entry.append(node('code','',item.command),node('span','command-explanation',explainCommand(item.command)));list.append(entry);});card.append(list);}themes.append(card); });
  document.querySelector('#curiosity-section').hidden = !visibleThemes.length;
