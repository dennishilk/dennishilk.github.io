const coreProcesses=interactiveUser=>[
...
export const createProcesses=(interactiveUser='visitor')=>interactiveUser==='m.weber'?coreProcesses(interactiveUser):[
 ...coreProcesses(interactiveUser).slice(0,-1),
 {pid:412,user:'root',cpu:0.0,mem:0.5,command:'sshd: /usr/sbin/sshd -D',name:'sshd',protected:true,service:'ssh.service',tty:'?'},
 {pid:420,user:'root',cpu:0.0,mem:0.4,command:'nginx: master process /usr/sbin/nginx',name:'nginx',protected:true,service:'nginx.service',tty:'?'},
 {pid:421,user:'www-data',cpu:0.1,mem:0.7,command:'nginx: worker process',name:'nginx',protected:true,service:'nginx.service',tty:'?'},
 {pid:455,user:'monitor',cpu:0.1,mem:0.8,command:'/opt/monitor/monitor',name:'monitor',protected:true,service:'monitor.service',tty:'?'},
 coreProcesses(interactiveUser).at(-1)
];
