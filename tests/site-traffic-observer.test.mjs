...
test('novel v1 baseline 2400 is rebaselined once to 26317 without adding old counted_since_cutover', async () => {
  ...
  const { writeTrafficPayload } = await import('../scripts/site-traffic-observer.mjs');
  const dir = await mkdtemp(join(tmpdir(), 'site-traffic-rebaseline-'));
  try {
    const output = join(dir, 'data', 'site-traffic.json');
    const stateFile = join(dir, 'state', 'site-traffic-total.json');
    const log = join(dir, 'access.log');
    const chapter = '/lost-administrator/novel/chapters/day-zero/';
    await writeFile(log, `${line({ at, path: chapter })}\n${line({ at: '08/Jul/2026:11:31:00 +0200', path: chapter })}\n`);
...
test('novel all-time copytruncate rotation checkpoints shrink without recounting retained content', async () => {
  const { mkdtemp, writeFile, appendFile, truncate, rm } = await import('node:fs/promises');
  const { tmpdir } = await import('node:os');
  const { join } = await import('node:path');
  const { writeTrafficPayload } = await import('../scripts/site-traffic-observer.mjs');
  const dir = await mkdtemp(join(tmpdir(), 'site-traffic-copytruncate-'));
  try {
    const output = join(dir, 'data', 'site-traffic.json');
    const stateFile = join(dir, 'state', 'site-traffic-total.json');
    const log = join(dir, 'access.log');
...
test('novel today and 24h windows stay distinct across Europe/Berlin midnight', () => {
  const chapter = '/lost-administrator/novel/chapters/day-zero/';
  const payload = buildTrafficPayload([
    line({ at: '08/Jul/2026:23:30:00 +0200', path: chapter }),
