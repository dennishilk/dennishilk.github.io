...
import { CANON_MAIL_IDS, CANON_MAIL_MESSAGES, MAIL_MESSAGES, MAIL_ACCOUNT } from '../assets/js/lost-administrator/mail-data.js';
...
test('live mailbox preserves the exact Day Zero Emma canon',()=>{
  ...
  assert.deepEqual(CANON_MAIL_MESSAGES.map(message=>message.id),CANON_MAIL_IDS);
  const [original,michael,emmaReply]=CANON_MAIL_MESSAGES;
  ...
  assert.doesNotMatch(JSON.stringify(CANON_MAIL_MESSAGES),/2026-07-29/);
  ...
  assert.doesNotMatch(text(c.execute('list')),/No messages in INBOX/);
  c.execute('folder sent');
  assert.doesNotMatch(text(c.execute('list')),/No messages in SENT/);
  assert.doesNotMatch(JSON.stringify(CANON_MAIL_MESSAGES),/Steve|preview deck|\.pptx/i);
});
...
test('opening the canonical Emma thread marks all replies read and reset restores defaults',()=>{const c=new MailClient(defaultWorkstationState());c.execute('folder emma');assert.equal(c.isUnread(CANON_MAIL_MESSAGES[2]),true);c.execute('open EMMA0731');assert.ok(CANON_MAIL_MESSAGES.every(message=>!c.isUnread(message)));const reset=new MailClient(defaultWorkstationState());assert.equal(reset.isUnread(CANON_MAIL_MESSAGES[2]),true);});
