...
test('live mailbox contains the Day Zero mail canon',()=>{
  ...
  const [original,michael,emmaReply,steve]=MAIL_MESSAGES;
  ...
  assert.equal(steve.subject,'Customer Status Experience — Concept Preview');
  ...
  assert.match(text(c.execute('open STEVE0730')),/From: Steve Harper[\s\S]*To: Henry Sullivan[\s\S]*Cc: Michael Weber <michael.weber@chesapeakesignaltech.com>/);
  assert.match(text(c.execute('attachments')),/customer-status-experience-concept-preview\.pptx/);
});
